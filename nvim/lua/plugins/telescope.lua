return {
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>fg", builtin.live_grep, { desc = "Telescope [F]iles live [G]rep" })

      vim.keymap.set("n", "<leader>fb", builtin.buffers, { desc = "Telescope [F]ind [B]uffer" })

      vim.keymap.set("n", "<leader>fs", builtin.git_status, { desc = "Telescope [F]ind git [S]tatus" })
